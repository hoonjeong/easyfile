...
const SvgConverter = () => {
  ...
  const handleFileSelect = useCallback(async (selectedFile) => {
    ...
    // Create sanitized preview URL to prevent XSS from malicious SVG
    try {
      const safeUrl = await createSafeSvgPreviewUrl(selectedFile);
      setPreviewUrl(safeUrl);
    } catch (err) {
      console.error('SVG sanitization error:', err);
      setError(t('validation.invalidSvg'));
    }
  }, [previewUrl, t]);
          ...
          <>
            ...
            <div className="options">
              ...
              <div className="option-group">
                <label className="option-label">{t('image.svg.scale')}</label>
                <select className="option-select" value={scale} onChange={(e) => setScale(Number(e.target.value))}>
                  <option value={1}>{t('image.svg.scale1x')}</option>
                  <option value={2}>{t('image.svg.scale2x')}</option>
                  <option value={3}>{t('image.svg.scale3x')}</option>
                  <option value={4}>{t('image.svg.scale4x')}</option>
                </select>
              </div>
            </div>
            ...
            {result && (
              <div className="result">
                ...
                <button className="download-button" onClick={handleDownload}>{t('image.downloadPng')}</button>
              </div>
            )}
